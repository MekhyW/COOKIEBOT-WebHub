...
export default function SimpleDropdown({
        ...
        if (isVisible && dropdownRef.current && !hasCalculated) {
            requestAnimationFrame(() => {
                ...
                const adjustments: [boolean, React.CSSProperties][] = [
                    [dropdownRect.right > viewportWidth, { right: 22, left: 'auto', translate: "0 8px" }],
                    [dropdownRect.bottom > viewportHeight, { bottom: 22, top: 'auto', translate: "0 8px" }],
                    [dropdownRect.top < 0, { top: 22, translate: "0 8px" }],
                    [dropdownRect.left < 0, { left: 22, translate: "0 8px" }]
                ];
    ...
    return (
        <div className={` transition-opacity ${ disabled && 'opacity-50 pointer-events-none' }`}>
            <button className={`${type} ${isVisible ? 'active' : 'active'}`} onClick={() => setIsVisible(prev => !prev)}>
                {buttonText}
                {icon}
            </button>
